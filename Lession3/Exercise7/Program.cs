...
namespace Exercise7
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IEnumerable<User> userresult = (from A in listuser
                                        where A.Adress == "Bratislava"
                                        select A);

            userresult.ToList().ForEach(A => Console.WriteLine(A.Name));
            Console.ReadKey();
            ...
            IEnumerable<Cars> carresult = (from A in carlist
                                        where A.cost > 30000 && A.cost < 100000
                                        select A);

            carresult.ToList().ForEach(A => Console.WriteLine(A.Name));
            Console.ReadKey();
            ...
            IEnumerable<Employees> employeesresult = (from Employee in userlist
                                             orderby Employee.LName ascending, Employee.Salary ascending
                                             select Employee);


            employeesresult.ToList().ForEach(Employee => Console.WriteLine($"{Employee.LName}, {Employee.Salary}"));
            //Console.ReadKey();
